 ...
 */
package com.arto.amq.annotation.parse;
...
public class AmqAnnotationParse implements MqParseStrategy {
    ...
    public void parseProducer(Object bean, Field field) {
        ...
        // 绑定生产者
        AmqProducerConfig config = new AmqProducerConfig(topic, annotation.priority());
        if (!clz.equals(Producer.class)) {
            // 不等于null 以"Producer.class"代替null
            log.warn("Activemq doesn't support callback in producer.");
        }
    ...
    public void parseConsumer(Set<String> keySet, Object bean, Method method) {
        ...
        // 重复注解
        String str = MqTypeEnum.ACTIVEMQ.getMemo() + topic + selectKey;
        if (keySet.contains(str) || keySet.contains(MqTypeEnum.ACTIVEMQ.getMemo() + topic)) {
            throw new MqClientException("Duplicated definition: @Consumer(type=activemq" + "', topic='" + topic + "', selectKey='" + selectKey + "')");
        }
        ...
        // 批量提交消费标识大小
        int ackSize = annotation.batchSize();
        if (ackSize <= 0) {
